...
public class DcController {
    ...
    public Object download(Integer status,String begin_date,String end_date,
        ...
        try{
            ...
            map.put("b_uid",users.getId());//查询摄影师的预约单
            map.put("status",1);
            ...
            String[] title = {"ID","账号","密码","名称","手机号","邮箱","性别"};
            // 创建Excel文件
            ...
            try {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    label = new Label(1, i + 1, u.getName());//预约人
                    sheet.addCell(label);

                    label = new Label(2, i + 1, u.getPwd());//预约时间
                    sheet.addCell(label);

                    label = new Label(3, i + 1,u.getNickname());//状态
                    sheet.addCell(label);


                    label = new Label(4, i + 1,u.getTel());//电话
                    sheet.addCell(label);



//                    label = new Label(5, i + 1,u.getEmail());//电话
                    sheet.addCell(label);
                    ...
                    label = new Label(6, i + 1,sex);//状态
                    sheet.addCell(label);



                    label = new Label(7, i + 1,u.getAcademy());//状态
                    sheet.addCell(label);


                    label = new Label(8, i + 1,u.getClass_grade());//状态
                    sheet.addCell(label);


                    label = new Label(9, i + 1,u.getBirthplace());//状态
                    sheet.addCell(label);
